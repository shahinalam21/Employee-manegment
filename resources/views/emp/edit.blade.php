...
@section('content')
    <div class="row">
        ...
        <div class="col-md-6 m-auto my-4">
            <div class="card">
                <h2 class="text-center bg-dark text-light py-2">Edit Employe</h2>
                <div class="card-body">
                    <form action="{{ url('/emp/store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-3">
                            <label for="exampleInputEmail1" class="form-label">Name</label>
                            <input type="text" name="name" class="form-control" value="{{$emps->name}}">
                            @error('name')
                        ...
                        <div class="mb-3">
                            <select name="post"  value="{{$emps->post}}" class="form-select">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="selory" name="selory" class="form-control"  value="{{$emps->selory}}">
                            @error('selory')
                                <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <input class="form-control" name="image" type="file" id="formFile"  value="{{$emps->image}}">
                            @error('image')
                                <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
